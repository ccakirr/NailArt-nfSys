...
def save_user(user_type, username, password, obj):
    ...
    if user_type == "manager":
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (user_type, username, password, obj.getDetails()['name'], obj.getDetails()['surname'],
              obj.getDetails()['mail'], obj.getDetails()['phone'], obj.getOffDay(), 0.0))
    elif user_type == "artist":
        cursor.execute("""
            INSERT INTO users (user_type, username, password, name, surname, email, phone, off_day, earnings)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (user_type, username, password, obj.getDetails()['name'], obj.getDetails()['surname'],
              obj.getDetails()['mail'], obj.getDetails()['phone'], obj.getDetails()['offDay'], obj.getDetails()['earnings']))
    elif user_type == "customer":
        cursor.execute("""
            INSERT INTO users (user_type, username, password, name, surname, email, phone, off_day, earnings)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (user_type, username, password, obj.getDetails()['name'], obj.getDetails()['surname'],
              obj.getDetails()['mail'], obj.getDetails()['phone'], None, 0.0))
